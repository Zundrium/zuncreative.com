...
export class HeroWave3D implements IBabylonGraphics {
	...
	private textureSamplerIndexTarget: number = 0;
	...
	public activateStep(index: number): void {
		...
		this.textureSamplerIndexTarget = index;
	}

	public resetSteps(): void {
		this.textureSamplerIndexTarget = 0;
	}
	...
	private getNoiseValue(x: number, y: number, elapsedTime: number): number {
		if (this.textureSamplerIndex !== this.textureSamplerIndexTarget) {
		...
		return this.textureSamplers[this.textureSamplerIndexTarget].getFloatAt(
			x,
			y,
			elapsedTime,
		);
	...
	private createLight(scene: Scene): void {
		new HemisphericLight("light", Vector3.Zero(), scene);
	}
	...
	private lerptextureSamplerIndex(): void {
		if (this.textureSamplerIndex != this.textureSamplerIndexTarget) {
			this.textureSamplerIndex = this.lerp(
				this.textureSamplerIndex,
				this.textureSamplerIndexTarget,
				1 * this.deltaTime,
			);
	...
	public async initialize(renderCanvas: HTMLCanvasElement): Promise<void> {
		...
		this.babylonScene.onRender = (delta: number) => {
			...
			if (this.fpsCounterElement) {
				this.fpsCounterElement.innerText = `FPS: ${this.babylonScene!.engine.getFps().toFixed()}`;
			}

			this.lerptextureSamplerIndex();
