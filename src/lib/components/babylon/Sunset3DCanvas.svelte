...
onMount(async () => {
	const Sunset3D = (await import("./Sunset3D")).Sunset3D;
	babylonGraphics = new Sunset3D();
...
export function makeShapeKeyActive(index: number) {
	babylonGraphics?.makeShapeKeyActive(index);
}
export function resetShapeKeys() {
	babylonGraphics?.resetShapeKeyTargetValues();
}
